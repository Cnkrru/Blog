...
import { t as ArticleCount_default } from "./ArticleCount-C4gjL8WI.js";
...
var TagCloud_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(mergeProps({ class: ["tag-cloud-container", { "dark-theme": isDarkTheme.value }] }, _attrs))} data-v-39672981><button class="tag-cloud-button" data-v-39672981><span class="button-icon" data-v-39672981>🏷️</span><span class="button-text" data-v-39672981>标签云</span><span class="tag-count" data-v-39672981>${ssrInterpolate(tags.value.length)}</span></button>`);
			if (showTagCloud.value) {
				_push(`<div class="tag-cloud-modal" data-v-39672981><div class="${ssrRenderClass([{ "dark-theme": isDarkTheme.value }, "tag-cloud-content"])}" data-v-39672981><div class="tag-cloud-header" data-v-39672981><h3 data-v-39672981>标签云</h3><button class="close-button" data-v-39672981>×</button></div><div class="search-container" data-v-39672981><input${ssrRenderAttr("value", searchQuery.value)} type="text" placeholder="搜索标签..." class="search-input" data-v-39672981>`);
				if (searchQuery.value) _push(`<button class="clear-button" data-v-39672981> × </button>`);
				else _push(`<!---->`);
				_push(`</div><div class="sort-options" data-v-39672981><span class="sort-label" data-v-39672981>排序:</span><button class="${ssrRenderClass(["sort-button", { active: sortBy.value === "frequency" }])}" data-v-39672981> 频率 </button><button class="${ssrRenderClass(["sort-button", { active: sortBy.value === "count" }])}" data-v-39672981> 数量 </button><button class="${ssrRenderClass(["sort-button", { active: sortBy.value === "recent" }])}" data-v-39672981> 最近 </button><button class="${ssrRenderClass(["sort-button", { active: sortBy.value === "trending" }])}" data-v-39672981> 趋势 </button></div><div class="tag-cloud" data-v-39672981><!--[-->`);
				ssrRenderList(filteredTags.value, (stat) => {
					...
					})}"${ssrRenderAttr("title", `频率: ${stat.frequency.toFixed(2)}, 数量: ${stat.count}, 最后使用: ${new Date(stat.lastUsed).toLocaleDateString()}`)} data-v-39672981>${ssrInterpolate(stat.tag)} (${ssrInterpolate(stat.count)}) </span>`);
				});
				_push(`<!--]--></div>`);
				if (selectedTag.value && tagArticles.value.length > 0) {
					_push(`<div class="tag-articles" data-v-39672981><div class="tag-header" data-v-39672981><h4 data-v-39672981>${ssrInterpolate(selectedTag.value)} 的文章 (${ssrInterpolate(tagArticles.value.length)})</h4>`);
					if (getRelatedTags(selectedTag.value).length > 0) {
						_push(`<div class="related-tags" data-v-39672981><span class="related-label" data-v-39672981>相关标签:</span><!--[-->`);
						ssrRenderList(getRelatedTags(selectedTag.value), (related) => {
							...
							})}"${ssrRenderAttr("title", `相关性: ${related.score.toFixed(2)}`)} data-v-39672981>${ssrInterpolate(related.tag)}</span>`);
						});
						_push(`<!--]--></div>`);
					} else _push(`<!---->`);
					_push(`</div><!--[-->`);
					ssrRenderList(tagArticles.value, (article) => {
						_push(ssrRenderComponent(unref(RouterLink), {
							...
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) _push(`<span class="tag-article-title" data-v-39672981${_scopeId}>${ssrInterpolate(article.title)}</span><span class="tag-article-date" data-v-39672981${_scopeId}>${ssrInterpolate(article.date)}</span>`);
				...
				if (selectedTag.value && tagArticles.value.length === 0) _push(`<div class="tag-articles-empty" data-v-39672981><span data-v-39672981>该标签下暂无文章</span></div>`);
				else _push(`<!---->`);
				_push(`</div></div>`);
			} else _push(`<!---->`);
			_push(`</div>`);
...
var TagCloud_default = /* @__PURE__ */ _plugin_vue_export_helper_default(TagCloud_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-39672981"]]);
//#endregion
