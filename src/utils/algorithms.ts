...
export class ElasticsearchLikeScorer {
  ...
  private bm25Params: { k1: number; b: number; avgFieldLengths: Map<string, { sum: number; count: number }> }
  private invertedIndex: Map<string, Map<string, Record<string, number>>>
  private fieldLengths: Map<string, Record<string, number>>
  private totalDocs: number
  private fieldDocFreq: Map<string, Map<string, Set<string>>>
  ...
  buildInvertedIndex(documents: any[]) {
  ...
  private tokenizeField(text: any, field: string): string[] {
    if (field === 'tags' && Array.isArray(text)) {
      return text.map((tag: string) => tag.toLowerCase())
    }
  ...
  private updateAvgFieldLength(field: string, length: number) {
  ...
  private updateDocumentFrequency(term: string, field: string, docId: string) {
  ...
  private calculateDocumentFrequency(term: string, field: string): number {
    const termMap = this.fieldDocFreq.get(field)
    if (!termMap || !termMap.has(term)) return 0
    return termMap.get(term)!.size
  }

  private score(docId: string, queryTerms: { term: string; weight: number }[]): number {
    let totalScore = 0
  ...
  search(query: string, documents: any[], limit: number = 20): any[] {
