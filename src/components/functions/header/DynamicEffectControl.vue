<template>
  <div class="dynamic-effect-control">
    <div
    ...
    >
      <img
        ...
        :style="{
          filter: isDarkMode ? 'brightness(0) contrast(2) drop-shadow(0 0 2px rgba(0,0,0,0.5))' : 'brightness(0) invert(1) contrast(2) drop-shadow(0 0 2px rgba(255,255,255,0.5))'
        }"
        alt="切换动态效果"
      />
    </div>
...
const initSakura = () => {
  ...
  if (!scriptLoaded) {
    ...
    script.src = '/assets/sakuraPlus.js'
    script.onload = () => {
      scriptLoaded = true
      setTimeout(() => {
